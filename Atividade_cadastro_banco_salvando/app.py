...
# Verificar conexão e criar tabelas se não existirem
try:
    with app.app_context():
        ...
        print(f"📊 Dados existentes - Usuários: {users_count}, Produtos: {products_count}")
...
def register():
    if request.method == 'POST':
        ...
        # Criar novo usuário COM COMMIT EXPLÍCITO
        hashed_password = generate_password_hash(password)
        new_user = User(username=username, email=email, password=hashed_password)
        
        try:
            db.session.add(new_user)
            db.session.flush()  # Força o ID a ser gerado
            db.session.commit()  # COMMIT EXPLÍCITO
            print(f"✅ Usuário cadastrado PERMANENTEMENTE: {username} (ID: {new_user.id})")
            
            # Verificar se realmente salvou
            user_check = User.query.filter_by(username=username).first()
            if user_check:
                print(f"✅ Confirmação: Usuário {user_check.username} salvo no banco!")
            else:
...
def add_product():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.flush()  # Força o ID a ser gerado
            db.session.commit()  # COMMIT EXPLÍCITO
            print(f"✅ Produto cadastrado PERMANENTEMENTE: {product_name} (ID: {new_product.id})")
            
            # Verificar se realmente salvou
            product_check = Product.query.filter_by(name=product_name).first()
            if product_check:
                print(f"✅ Confirmação: Produto {product_check.name} salvo no banco!")
            else:
                print("❌ ERRO: Produto não foi salvo!")
